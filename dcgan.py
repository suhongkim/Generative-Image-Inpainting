...
def main():
    ...
    #random horizontal and vertical flip, color jitter, random rotation, random resizing and random crop.
    ...
    transforms_aug = transforms.Compose([
            transforms.Resize( ( opt.img_size-1, opt.img_size-1) ), # Zoom a bit
            transforms.RandomHorizontalFlip(p=0.5), 
            transforms.ColorJitter(
                brightness=random.uniform(0.0, 0.2), 
                contrast=random.uniform(0.0, 0.2), 
                saturation=random.uniform(0.0, 0.2), 
                hue=random.uniform(0.0, 0.05)
            ), 
            transforms.RandomRotation(degrees=(-5, 5)), 
            transforms.RandomResizedCrop(opt.img_size, scale=(0.8, 1.0)),
	        transforms.ToTensor(),       
